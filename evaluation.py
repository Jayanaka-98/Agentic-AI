...
def evaluate_results(exp_type: str):
    ...
    print(f"[OK] {exp_type}: evaluated {num_q} questions")
...
# -------------------------------------------------
# New: side-by-side comparison per metric
# -------------------------------------------------
def visualize_overall_comparison(results):
    ...
    # --- subplot 1: Macro F1 ---
    plt.subplot(1, 3, 1)
    plt.bar(labels, macro_f1, color="steelblue")
    plt.title("Macro F1")
    plt.ylim(0, 1)
    ...
    # --- subplot 2: Micro F1 ---
    plt.subplot(1, 3, 2)
    plt.bar(labels, micro_f1, color="darkorange")
    plt.title("Micro F1")
    plt.ylim(0, 1)
    plt.grid(axis="y", linestyle="--", alpha=0.5)
    ...
    plt.subplot(1, 3, 3)
    plt.bar(labels, cosine_pred, color="seagreen")
    ...
    print(f"[OK] Saved side-by-side comparison chart → {out_path}")
